...
from src.evaluation import (
    cause_effect_exact_match,
    cause_effect_semantic_match,
    run_evaluation
)
...
df_dev = load_train_data("./data/train_data.json")
dev_examples = []
...
if st.button("Load Model for Evaluation"):
    ...
    avg_exact, scores_exact = evaluator(
        model, metric=cause_effect_exact_match, return_all_scores=True
    )
    ...
    avg_semantic, scores_semantic = evaluator(
        model, metric=cause_effect_semantic_match, return_all_scores=True
    )
